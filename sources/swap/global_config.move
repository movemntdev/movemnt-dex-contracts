...
module movement_dex::global_config {
    use std::signer;
    ...
    public fun get_dao_admin(): address acquires GlobalConfig {
        assert!(exists<GlobalConfig>(@movement_dex), ERR_CONFIG_DOES_NOT_EXIST);

        let config = borrow_global<GlobalConfig>(@movement_dex);
    ...
    public entry fun set_dao_admin(dao_admin: &signer, new_addr: address) acquires GlobalConfig {
        ...
        let config = borrow_global_mut<GlobalConfig>(@movement_dex);
    ...
    public entry fun set_default_fee<Curve>(fee_admin: &signer, default_fee: u64) acquires GlobalConfig, EventsStore {
        ...
        let events_store = borrow_global_mut<EventsStore>(@movement_dex);
        if (curves::is_stable<Curve>()) {
    ...
    public entry fun set_default_dao_fee(fee_admin: &signer, default_fee: u64) acquires GlobalConfig, EventsStore {
        ...
        let event_store = borrow_global_mut<EventsStore>(@movement_dex);
        event::emit_event(
    ...
    public fun initialize_for_test() {
        let liquidswap_admin = aptos_framework::account::create_account_for_test(@movement_dex);
