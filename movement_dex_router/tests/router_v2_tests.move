#[test_only]
module movement_dex::router_v2_tests {
    use std::signer;
    ...
    fun test_add_initial_liquidity() {
        ...
        let (coin_x, coin_y, lp_coins) =
            router_v2::add_liquidity<BTC, USDT, Uncorrelated>(
                btc_coins,
                101,
                usdt_coins,
                10100,
    ...
    fun test_cannot_add_liquidity_to_pool_in_reverse_order() {
        ...
        let (coin_y, coin_x, lp_coins) =
            router_v2::add_liquidity<USDT, BTC, Uncorrelated>(
                usdt_coins,
                9000,
                btc_coins,
                10,
    ...
    fun test_add_liquidity_to_fail_with_insufficient_x_coins() {
        ...
        let (coin_y, coin_x, lp_coins) = router_v2::add_liquidity<BTC, USDT, Uncorrelated>(
            btc_coins,
            102,
            usdt_coins,
            9000,
    ...
    fun test_remove_liquidity() {
        ...
        let (x_out, y_out) = router_v2::get_reserves_for_lp_coins<BTC, USDT, Uncorrelated>(
            lp_coins_val
        );
        let (coin_x, coin_y) =
            router_v2::remove_liquidity<BTC, USDT, Uncorrelated>(lp_coins_to_burn, x_out, y_out);

        let (usdt_reserve, btc_reserve) = router_v2::get_reserves_size<USDT, BTC, Uncorrelated>();
    ...
    fun test_remove_liquidity_to_fail_if_less_than_minimum_x() {
        ...
        let (coin_x, coin_y) =
            router_v2::remove_liquidity<BTC, USDT, Uncorrelated>(lp_coins_to_burn, x_out * 2, y_out);
    ...
    fun test_remove_liquidity_to_fail_if_less_than_minimum_y() {
        ...
        let (coin_x, coin_y) =
            router_v2::remove_liquidity<BTC, USDT, Uncorrelated>(lp_coins_to_burn, x_out, y_out * 2);
    ...
    fun test_swap_exact_coin_for_coin() {
        ...
        let usdt_amount_out = router_v2::get_amount_out<BTC, USDT, Uncorrelated>(btc_coins_swap_val);

        let usdt_coins = router_v2::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
            btc_coins_to_swap,
            usdt_amount_out,
        );
    ...
    fun test_swap_exact_coin_for_coin_1() {
        ...
        let usdt_to_get_val = router_v2::get_amount_out<BTC, USDT, Uncorrelated>(btc_to_swap_val);
    ...
    fun test_swap_exact_coin_for_coin_2() {
        ...
        let btc_to_get_val = router_v2::get_amount_out<USDT, BTC, Uncorrelated>(usdt_to_swap_val);

        let usdt_coins = router_v2::swap_exact_coin_for_coin<USDT, BTC, Uncorrelated>(
            usdt_to_swap,
            btc_to_get_val,
        );
    ...
    fun test_swap_exact_coin_for_coin_reverse() {
        ...
        let btc_coins = router_v2::swap_exact_coin_for_coin<USDT, BTC, Uncorrelated>(
            usdt_coins_to_swap,
            1,
        );
    ...
    fun test_swap_coin_for_exact_coin() {
        ...
        let (remainder, usdt_coins) =
            router_v2::swap_coin_for_exact_coin<BTC, USDT, Uncorrelated>(
                btc_coins_to_swap,
                98,
            );
    ...
    fun test_swap_coin_for_exact_coin_1() {
        ...
        let btc_coins_to_swap_val = router_v2::get_amount_in<BTC, USDT, Uncorrelated>(usdt_coins_to_get);
    ...
    fun test_swap_coin_for_exact_coin_2() {
        ...
        let usdt_coins_to_swap_val = router_v2::get_amount_in<USDT, BTC, Uncorrelated>(btc_coins_to_get);

        let usdc_coins_to_swap = test_coins::mint<USDT>(&coin_admin, usdt_coins_to_swap_val);

        let (remainder, btc_coins) =
            router_v2::swap_coin_for_exact_coin<USDT, BTC, Uncorrelated>(
                usdc_coins_to_swap,
                btc_coins_to_get,
            );
    ...
    fun test_swap_coin_for_exact_coin_router_check_fails() {
        ...
        let btc_coins_to_swap = test_coins::mint<BTC>(&coin_admin, btc_coins_to_swap_val);
        let usdt_to_get = router_v2::get_amount_out<BTC, USDT, Uncorrelated>(btc_coins_to_swap_val) + 1;
    ...
    fun test_fail_if_price_fell_behind_threshold() {
        ...
        let usdt_coins =
            router_v2::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
                btc_coin_to_swap,
                102,
            );
    ...
    fun test_returned_usdt_proportially_decrease_for_big_swaps() {
        ...
        let (btc_reserve, usdt_reserve) = router_v2::get_reserves_size<BTC, USDT, Uncorrelated>();
        assert!(btc_reserve == 301, 1);
        assert!(usdt_reserve == 3396, 2);
        assert!(router_v2::current_price<USDT, BTC, Uncorrelated>() == 11, 3);
    ...
    fun test_pool_exists() {
        ...
        assert!(router_v2::is_swap_exists<BTC, USDT, Uncorrelated>(), 0);
        assert!(router_v2::is_swap_exists<USDT, BTC, Uncorrelated>(), 1);
    }
    ...
    fun test_cumulative_prices_after_swaps() {
        ...
        let (btc_price, usdt_price, ts) =
            router_v2::get_cumulative_prices<BTC, USDT, Uncorrelated>();
    ...
    fun test_stable_curve_swap_exact() {
        ...
        let usdt_to_get = router_v2::get_amount_out<USDC, USDT, Stable>(usdc_to_swap_val);

        let usdt_swapped = router_v2::swap_exact_coin_for_coin<USDC, USDT, Stable>(
            usdc_to_swap,
            usdt_to_get,
        );
    ...
    fun test_stable_curve_swap_exact_vice_versa() {
        ...
        let usdc_to_get_val = router_v2::get_amount_out<USDT, USDC, Stable>(usdt_to_swap_val);
        let usdt_to_swap = test_coins::mint<USDT>(&coin_admin, usdt_to_swap_val);

        let usdc_swapped = router_v2::swap_exact_coin_for_coin<USDT, USDC, Stable>(
            usdt_to_swap,
            usdc_to_get_val,
        );
    ...
    fun test_stable_curve_exact_swap() {
        ...
        let usdt_to_swap_val = router_v2::get_amount_in<USDT, USDC, Stable>(usdc_to_get_val);

        let usdt_to_swap = test_coins::mint<USDT>(&coin_admin, usdt_to_swap_val);
        let (usdt_reminder, usdc_swapped) = router_v2::swap_coin_for_exact_coin<USDT, USDC, Stable>(
            usdt_to_swap,
            usdc_to_get_val,
        );
    ...
    fun test_stable_curve_exact_swap_vice_versa() {
        ...
        let usdc_to_swap_val = router_v2::get_amount_in<USDC, USDT, Stable>(usdt_to_get_val);

        let usdc_to_swap = test_coins::mint<USDC>(&coin_admin, usdc_to_swap_val);
        let (usdc_reminder, usdt_swapped) = router_v2::swap_coin_for_exact_coin<USDC, USDT, Stable>(
            usdc_to_swap,
            usdt_to_get_val,
        );
    ...
    fun test_stable_get_amount_in() {
        ...
        let amount_in = router_v2::get_amount_in<USDC, USDT, Stable>(67279092);
    ...
    fun test_get_amount_in() {
        ...
        let amount_in = router_v2::get_amount_in<USDT, BTC, Uncorrelated>(158202011);
    ...
    fun test_get_amount_in_1() {
        ...
        let amount_in = router_v2::get_amount_in<BTC, USDT, Uncorrelated>(28253021000);
    ...
    fun test_get_amount_in_2() {
        ...
        let amount_in = router_v2::get_amount_in<USDT, BTC, Uncorrelated>(1);
    ...
    fun test_stable_get_amount_in_2() {
        ...
        let amount_in = router_v2::get_amount_in<USDC, USDT, Stable>(15000);
    ...
    fun test_stable_get_amount_in_3() {
        ...
        let amount_in = router_v2::get_amount_in<USDT, USDC, Stable>(158282982);
    ...
    fun test_stable_get_amount_in_4() {
        ...
        let amount_in = router_v2::get_amount_in<USDT, USDC, Stable>(1);
    ...
    fun test_stable_get_amount_in_5() {
        ...
        let amount_in = router_v2::get_amount_in<USDT, USDC, Stable>(57212828231);
    ...
    fun test_stable_get_amount_out() {
        ...
        let amount_out = router_v2::get_amount_out<USDC, USDT, Stable>(674816);
    ...
    fun test_get_amount_out() {
        ...
        let amount_out = router_v2::get_amount_out<USDT, BTC, Uncorrelated>(1500000000);
    ...
    fun test_get_amount_out_1() {
        ...
        let amount_out = router_v2::get_amount_out<BTC, USDT, Uncorrelated>(100000000);
    ...
    fun test_get_amount_out_2() {
        ...
        let amount_out = router_v2::get_amount_out<BTC, USDT, Uncorrelated>(1);
    ...
    fun test_stable_get_amount_out_1() {
        ...
        let amount_out = router_v2::get_amount_out<USDC, USDT, Stable>(323859);
    ...
    fun test_stable_get_amount_out_2() {
        ...
        let amount_out = router_v2::get_amount_out<USDC, USDT, Stable>(58201);
    ...
    fun test_stable_get_amount_out_3() {
        ...
        let amount_out = router_v2::get_amount_out<USDT, USDC, Stable>(15000);
    ...
    fun test_stable_get_amount_out_4() {
        ...
        let amount_out = router_v2::get_amount_out<USDT, USDC, Stable>(1);
    ...
    fun test_stable_get_amount_out_5() {
        ...
        let amount_out = router_v2::get_amount_out<USDT, USDC, Stable>(572123482812);
    ...
    fun test_convert_with_current_price() {
        let a = router_v2::convert_with_current_price(MAX_U64, MAX_U64, MAX_U64);
        assert!(a == MAX_U64, 0);

        a = router_v2::convert_with_current_price(100, 100, 20);
        assert!(a == 20, 1);

        a = router_v2::convert_with_current_price(256, 8, 2);
        assert!(a == 64, 1);
    }
    ...
    fun test_get_decimals_scales_stables() {
        ...
        let (x, y) = router_v2::get_decimals_scales<USDC, USDT, Stable>();
    ...
    fun test_get_decimals_scales_uncorrelated() {
        ...
        let (x, y) = router_v2::get_decimals_scales<BTC, USDT, Uncorrelated>();
    ...
    fun test_swap_coin_for_coin_unchecked() {
        ...
        let usdt_coins = router_v2::swap_coin_for_coin_unchecked<BTC, USDT, Uncorrelated>(
            btc_coins_to_swap,
            usdt_amount_out,
        );
        assert!(coin::value(&usdt_coins) == usdt_amount_out, 0);
    ...
    fun test_stable_swap_coin_for_coin_unchecked() {
        ...
        let usdt_amount_out = router_v2::get_amount_out<USDC, USDT, Stable>(usdc_coins_swap_val);

        let usdt_coins = router_v2::swap_coin_for_coin_unchecked<USDC, USDT, Stable>(
            usdc_coins_to_swap,
            usdt_amount_out,
        );
        assert!(coin::value(&usdt_coins) == usdt_amount_out, 0);
    ...
    fun test_stable_swap_coin_for_coin_unchecked_reverse() {
        ...
        let usdt_to_swap = test_coins::mint<USDT>(&coin_admin, usdt_to_swap_val);
        let usdt_amount_out = router_v2::get_amount_out<USDT, USDC, Stable>(usdt_to_swap_val);

        let usdt_coins = router_v2::swap_coin_for_coin_unchecked<USDT, USDC, Stable>(
            usdt_to_swap,
            usdt_amount_out,
        );
        assert!(coin::value(&usdt_coins) == usdt_amount_out, 0);
    ...
    fun test_swap_coin_for_coin_unchecked_reverse() {
        ...
        let btc_coins = router_v2::swap_coin_for_coin_unchecked<USDT, BTC, Uncorrelated>(
            usdt_coins_to_swap,
            1,
        );
        assert!(coin::value(&btc_coins) == 1, 0);
    ...
    fun test_swap_coin_for_coin_unchecked_1() {
        ...
        let usdt_to_get_val = router_v2::get_amount_out<BTC, USDT, Uncorrelated>(btc_to_swap_val) - 1;
    ...
    fun test_swap_coin_for_coin_unchecked_2() {
        ...
        let btc_to_get_val = router_v2::get_amount_out<USDT, BTC, Uncorrelated>(usdt_to_swap_val) - 134567;

        let usdt_coins = router_v2::swap_coin_for_coin_unchecked<USDT, BTC, Uncorrelated>(
            usdt_to_swap,
            btc_to_get_val,
        );
        assert!(coin::value(&usdt_coins) == btc_to_get_val, 0);
    ...
    fun test_fail_if_price_fell_behind_threshold_unchecked() {
        ...
        let usdt_coins =
            router_v2::swap_coin_for_coin_unchecked<BTC, USDT, Uncorrelated>(
                btc_coin_to_swap,
                102,
            );
    ...
    fun test_stable_fail_if_price_fell_behind_threshold_unchecked_2() {
        ...
        let usdt_coins_to_swap_val = router_v2::get_amount_in<USDT, USDC, Stable>(usdc_coins_to_get);
        let usdt_coins_to_swap = test_coins::mint<USDT>(&coin_admin, usdt_coins_to_swap_val);

        let usdc_coins = router_v2::swap_coin_for_coin_unchecked<USDT, USDC, Stable>(
            usdt_coins_to_swap,
            usdc_coins_to_get + 1,
        );
    ...
    fun test_swap_coin_for_coin_unchecked_fails() {
        ...
        let btc_to_get_val = router_v2::get_amount_out<USDT, BTC, Uncorrelated>(usdt_to_swap_val) + 1;
    ...
    fun test_calc_optimal_coin_values() {
        ...
        let (x_value, y_value) = router_v2::calc_optimal_coin_values<BTC, USDT, Uncorrelated>(x_desired, y_desired, 0, 0);
    ...
    fun test_calc_optimal_coin_values_1() {
        ...
        let (_x_value, _y_value) = router_v2::calc_optimal_coin_values<BTC, USDT, Uncorrelated>(x_desired, y_desired, 5000000000, 0);
    }
    ...
    fun test_calc_optimal_coin_values_3() {
        ...
        let (x_res, y_res) = router_v2::get_reserves_size<BTC, USDT, Uncorrelated>();
    ...
    fun test_calc_optimal_coin_values_4() {
        ...
        let (_x_value, _y_value) = router_v2::calc_optimal_coin_values<BTC, USDT, Uncorrelated>(x_desired, y_desired, 0, 2800000000);
    }
    ...
    fun test_fee_config_for_uncorrelated_curve() {
        ...
        let (fee, d) = router_v2::get_fees_config<BTC, USDT, Uncorrelated>();
        assert!(fee == 30, 1);
        assert!(d == 10000, 2);
        let (fee, d) = router_v2::get_fees_config<USDT, BTC, Uncorrelated>();
        assert!(fee == 30, 3);
        assert!(d == 10000, 4);

        let fee = router_v2::get_fee<BTC, USDT, Uncorrelated>();
        assert!(fee == 30, 5);
        let fee = router_v2::get_fee<USDT, BTC, Uncorrelated>();
        assert!(fee == 30, 6);

        let (dao_fee, d) = router_v2::get_dao_fees_config<BTC, USDT, Uncorrelated>();
        assert!(dao_fee == 33, 7);
        assert!(d == 100, 8);
        let (dao_fee, d) = router_v2::get_dao_fees_config<USDT, BTC, Uncorrelated>();
        assert!(dao_fee == 33, 9);
        assert!(d == 100, 10);

        let dao_fee = router_v2::get_dao_fee<BTC, USDT, Uncorrelated>();
        assert!(dao_fee == 33, 11);
        let dao_fee = router_v2::get_dao_fee<USDT, BTC, Uncorrelated>();
    ...
    fun test_fee_config_for_stable_curve() {
        ...
        let (fee, d) = router_v2::get_fees_config<USDC, USDT, Stable>();
        assert!(fee == 4, 1);
        assert!(d == 10000, 2);
        let (fee, d) = router_v2::get_fees_config<USDT, USDC, Stable>();
        assert!(fee == 4, 3);
        assert!(d == 10000, 4);

        let fee = router_v2::get_fee<USDC, USDT, Stable>();
        assert!(fee == 4, 5);
        let fee = router_v2::get_fee<USDT, USDC, Stable>();
        assert!(fee == 4, 6);

        let (dao_fee, d) = router_v2::get_dao_fees_config<USDC, USDT, Stable>();
        assert!(dao_fee == 33, 7);
        assert!(d == 100, 8);
        let (dao_fee, d) = router_v2::get_dao_fees_config<USDT, USDC, Stable>();
        assert!(dao_fee == 33, 9);
        assert!(d == 100, 10);

        let dao_fee = router_v2::get_dao_fee<USDC, USDT, Stable>();
        assert!(dao_fee == 33, 11);
        let dao_fee = router_v2::get_dao_fee<USDT, USDC, Stable>();
        assert!(dao_fee == 33, 12);
    }
    ...
    fun test_get_amount_out_does_not_overflow_on_liquidity_close_to_u64_max() {
        ...
        let _ = router_v2::get_amount_out<BTC, USDT, Uncorrelated>(1);
    }
    ...
    fun test_get_amount_out_does_not_overflow_on_coin_in_close_to_u64_max() {
        ...
        let _ = router_v2::get_amount_out<BTC, USDT, Uncorrelated>(2658758714820000);
    }
    ...
    fun test_get_amount_in_does_not_overflow_on_liquidity_x_close_to_u64_max() {
        ...
        let _ = router_v2::get_amount_in<BTC, USDT, Uncorrelated>(1);
    }
    ...
    fun test_get_amount_in_aborts_if_coin_out_bigger_than_reserves() {
        ...
        let _ = router_v2::get_amount_in<BTC, USDT, Uncorrelated>(2658758714820000);
    }
    ...
    fun test_get_amount_in_if_coin_out_exactly_equals_reserve_out() {
        ...
        let _ = router_v2::get_amount_in<BTC, USDT, Uncorrelated>(28000000000);
    }
    ...
    fun test_add_liquidity_with_imbalanced_reserves() {
        ...
        let (btc_out, usdt_out, lp_out) =
            router_v2::add_liquidity<BTC, USDT, Uncorrelated>(btc_in, 1, usdt_in, 1);
